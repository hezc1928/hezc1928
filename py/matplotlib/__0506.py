...
def r_dim1_HG(T, L, pos=1):
    return T**4/TC**4*(Han.Han_p(T, L, pos)/Han.Han_p(T, 1/CRU, pos))
...
def r_dim1_HG_list(T, L, pos=1):
    ...
    for i in T:
        print("T = %.2f" % i)
        temp_1 = ((Han.Han_I(MI/i, pos=pos)) /
                  (Han.Han_I(MI/TC, pos=pos)))
        temp_2 = (i**4/TC**4)*(Han.Han_p(T=i, L=L, pos=pos) /
                               Han.Han_p(T=TC, L=1/CRU, pos=pos))
        temp_3 = (i**4/TC**4)
        rdim1_list.append(temp_3)
    return rdim1_list
...
def main():
    ...
    plt.plot(T, r_dim1_HG_list(T, 1/CRU, 1), label='inf_1')
