...
def f_2(L, pos=1):
    ...
    limit = 0
    while True:
        an = integrate.quad(f_2_minor, 0, np.inf, args=(L, k, pos))[0]
        s += an
        if limit == PRE//10:
            break
        if np.abs(an) <= 1/PRE:
...
def f_3(L, pos=1):
    ...
    limit = 0
    while True:
        an = integrate.quad(f_3_minor, 0, np.inf, args=(L, k, pos))[0]
        s += an
        if limit == PRE//10:
            break
        if np.abs(an) <= 1/PRE:
...
def f_5(L1, L2, pos=1):
    ...
    while True:
        ...
        s = 0
        while True:
            an = integrate.quad(f_5_minor, 0, np.inf, limit=10*PRE,
...
def P(L1, L2, pos=1):
    return 1+(1/L1+1/L2)*f_1(pos)+f_2(L1)+f_2(L2)+2*f_3(L1)/L2+2*f_3(L2)/L1+2*f_4_N(pos)/(L1*L2)*+2*f_5(L1, L2, pos)
...
def P_list_var(L, R, pos=1):
    ...
    Plist = []
    for i in L:
        L1.append(2/(1/i+1/R+0.00001))
        L2.append(2/(1/i-1/R+0.00001))
    print("finish")
    for i in range(len(L1)):
        # print("L1 = ", L1[i])
        # print("L2 = ", L2[i])
        # Plist.append(1+(1/L1[i]+1/L2[i])*f_1(pos)+f_2(L1[i])+f_2(L2[i])+2*f_3(L1[i])/L2[i]+2*f_3(L2[i])/L1[i])
...
def appro0503():
    ...
    for i in range(0, 10):
        ...
        a = pow
        var.append(pow)
        ori.append(integrate.quad(test, 0, np.inf, args=(a, pos))[0])
        ans.append(1/a*(1-np.e**(-np.pi/2/a))-np.pi/2/a/a*np.e**(-np.pi/2/a))
    plt.plot(var, ori, label='ori')
    plt.plot(var, ans, label='ans')
    # 可以看出这种估计的好坏


start = time.time()
# initialize()
x = np.arange(1/PRE, RANGE, 1/PRE)  # 最小值取零会报错

# print(f_1(1))
# print(f_1(-1))
# plt.plot(x, f_2(x), 'b-')
# plt.plot(x, f_2(x, -1), 'b-')
# plt.plot(x, f_3(x), 'r-')
# plt.plot(x, f_3(x, -1), 'r-')
# print(f_4(1))
# print(f_4(-1))
# plt.plot(x, f_5_list(x, 1, 1), 'r-')  # time =  1888.1135091781616
pi_show_fermi(x )
# plt.plot(x, P_list_var(x, 1000000, 1))
# plt.plot(x, P_list_var(x, 1, 1))
